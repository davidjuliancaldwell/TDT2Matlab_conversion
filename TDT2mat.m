...
if ~bUseOutsideTTX
    ...
    if TTX.SelectBlock(['~' block]) ~= 1
        ...
        while strcmp(block_name, '') == 0
            ...
            % Note: the above if-else statement used to be just below the
            % block_name line as written by TDT, but this caused the system
            % to return the wrong error if block #1 was found but couldn't
            % be opened. Moving the statement above seems to fix that issue
            % (so all errors returned are correct) and doens't introduce
            % any other bugs. Updated by JCronin 07/27/18 
        ...
        case 'Stream'
            ...
            if ranges_size > 0
                ...
                while nancheck
                    ...
                    warning('ReadWavetank.getBlockInfosV returned NaN for %s, attempting step size %.2f', name, step_size);
